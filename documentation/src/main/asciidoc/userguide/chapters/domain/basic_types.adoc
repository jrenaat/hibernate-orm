...
`optional` - boolean (defaults to true):: Defines whether this [.line-through]#attribute# field allows nulls.  Jakarta Persistence defines
    this annotation attribute as "a hint", which means the provider is free to ignore it.  Jakarta Persistence also says that it will be
...
`fetch` - FetchType (defaults to EAGER):: Defines whether this [.line-through]#attribute# field should be fetched eagerly or lazily.
...
map basic values to the database.
